...
const Header = () => {
  ...
  async function handleRegister() {
    try {
      await register(userEmail, userPass);
      setUserEmail("");
      setUserPass("");
      setModal(false);
    } catch (err) {
          ...
          {modal ? (
            <div className="">
              <div className="fixed 2xl:top-[150px]  max-xl:top-[50px] xl:left-[35%] max-lg:left-0 md:w-[480px] max-lg:w-[290px] lg:h-[800px] max-sm:h-[500px] p-6 z-50 bg-[#0f0e0ed8] px-[50px] flex items-start  justify-center flex-col md:gap-[30px] ">
                ...
                <div className="flex items-center flex-col gap-[15px] w-full">
                <button
                  ...
                  <h1 className="text-gray-300 text-xl">ИЛИ</h1>
                  <button
                    onClick={handleLogIn}
                    type="button"
                ...
                {error.length ? (
                  <div
                    class="flex items-center p-4 mb-4 text-xl  rounded-lg  text-[#E50914] w-full"
                    role="alert"
                  >
                    <svg
